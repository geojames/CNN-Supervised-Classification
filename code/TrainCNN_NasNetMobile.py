...
This file trains and outputs VGG16 to for the task of river classification.
"""
...
# 3 - Feeds this into the VGG16 convnet.  Level of trainability of the convnet can be set by user
...
#use this option to train the model with a validation subset. 
#Accuracy and loss checks will be displayed.
#When the tuning is satisfactory, set to False and train with the whole dataset.
#Model will only be saved if this is set to False
#When true the sript will exit with a system error and display loss/acc vs epoch figures.  
#This is intentional.
ModelTuning = True
TuningEpochs = 50 
TuningSubSamp = 0.5 # Subsample of data, 0-1, to be used in tuning.
...
###############################################################################
# Simplify local classes and return a class prediction to the 1-Nclasses hierarchical classes
...
for f in range(0,len(RiverTuple)):
    ...
    for i in range(0,BiggestImage): #Do not use so many images! 
...
##############################################################################
                '''Alter the class to have unique classes for each river'''
# The classes in the 1st river will remain the same, but after they will have Nclasses added.                
...
################################################################################            
            ...
            del(ImCrop,ClsCrop,I_tiles, C_tiles, Label1hot, Class, Im3D, CLabelVector, ClassifiedTiles)
...
conv_base = nasnet.NASNetMobile(weights='imagenet', include_top = False, input_shape = (size,size,3))
...
#Tune an optimiser
Optim = optimizers.Adam(lr=0.0005, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=True)
...
if ModelTuning:
    ...
    history = model.fit(Partiel_trainX, Partiel_trainY, epochs = TuningEpochs, batch_size = 30, validation_data = (Partiel_testX, Partiel_testY))
...
#Fit all the data for transfer learning
